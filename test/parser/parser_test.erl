...
root_test_() ->
  [ % 'grammar' and 'exp_list'
    ...
    ?_assertEqual([],                       reia:parse("")),
    ?_assertEqual([],                       reia:parse(" ")),
    ?_assertEqual([],                       reia:parse(" \n ")),
    ?_assertEqual([{integer,1,12}],         reia:parse("12 ")),
    ?_assertEqual([{integer,2,12}],         reia:parse(" \n12 ")),
    ?_assertEqual([{integer,1,12}],         reia:parse("12 \n ")),
    ?_assertEqual([{integer,2,12}],         reia:parse(" \n12\n ")),
...
expr_test_() ->    % NOTE: All depend on 'integer' already working
  ...
  [ % inline_if
    ...
    ?_assertEqual([{match,1,{integer,1,3},{binary_op,1,'+=',{integer,1,1},{integer,1,2}}}],  
                                            reia:parse("3 = 1 += 2")),
    ?_assertEqual([{binary_op,1,'+=',{integer,1,1},{integer,1,2}}],  reia:parse("1 += 2")),
    ?_assertEqual([{binary_op,1,'-=',{integer,1,1},{integer,1,2}}],  reia:parse("1 -= 2")),
    ?_assertEqual([{binary_op,1,'*=',{integer,1,1},{integer,1,2}}],  reia:parse("1 *= 2")),
    ?_assertEqual([{binary_op,1,'/=',{integer,1,1},{integer,1,2}}],  reia:parse("1 /= 2")),
    ?_assertEqual([{binary_op,1,'**=',{integer,1,1},{integer,1,2}}], reia:parse("1 **= 2")),
    ...
    ?_assertEqual([{binary_op,1,'<<=',{integer,1,1},{integer,1,2}}], reia:parse("1 <<= 2")),
    ?_assertEqual([{binary_op,1,'>>=',{integer,1,1},{integer,1,2}}], reia:parse("1 >>= 2")),
    ?_assertEqual([{binary_op,1,'>>=',{integer,1,1},{integer,2,2}}], reia:parse("1 >>= \n 2")),
...
basic_term_test_() -> %==========================================
  [ % integer
    ?_assertEqual([{integer,1,23}],         reia:parse("23 ")),
    % float
    ?_assertEqual([{float,1,23.4}],         reia:parse("23.4 ")),
    % atom
    ?_assertEqual([{atom,1,abc}],           reia:parse(":abc ")),
    ?_assertEqual([{atom,1,abc}],           reia:parse(":'abc' ")),
    ?_assertEqual([{atom,1,'a\'bc'}],       reia:parse(":'a\\'bc' ")),
    ?_assertEqual([{atom,1,'a"bc'}],        reia:parse(":\"a\\\"bc\" ")),
    % boolean values
    ?_assertEqual([{true,1}],               reia:parse("true ")),
    ?_assertEqual([{false,1}],              reia:parse("false ")),
    ?_assertEqual([{nil,1}],                reia:parse("nil ")),
    % regexp
    ?_assertEqual([{regexp,1,""}],          reia:parse("%r// ")),
    ?_assertEqual([{regexp,1,"a/bc"}],      reia:parse("%r/a\\/bc/ ")),
    ?_assertEqual([{regexp,1,"abc"}],       reia:parse("%r/abc/ ")),
    % self
    ?_assertEqual([{self,1}],               reia:parse("self ")),

    % module name
    ?_assertEqual([{module_name,1,'Mod'}],  reia:parse("Mod ")),
    ?_assertEqual([{module_name,1,'M0d_Ul3_'}],
                                            reia:parse("M0d_Ul3_ ")),

    % instance variable
    ?_assertEqual([{ivar,1,'ident'}],       reia:parse("@ident ")),
    % bound variable
    ?_assertEqual([{bound_var,1,'ident'}],  reia:parse("^ident ")),
    % identifier
    ?_assertEqual([{var,1,ident}],          reia:parse("ident ")),
    ?_assertEqual([{var,1,ident_name}],     reia:parse("ident_name ")),
    ?_assertEqual([{var,1,ident_3}],        reia:parse("ident_3 ")),
    ?_assertEqual([{var,1,iDeNt_4_}],       reia:parse("iDeNt_4_")),
    ?_assertEqual([{var,1,'_ident_name'}],  reia:parse("_ident_name")),
    % apostrophe string - simple
    ?_assertEqual([{string,1,""}],          reia:parse("'' ")),
    ?_assertEqual([{string,1,"abc"}],       reia:parse("'abc' ")),
    ?_assertEqual([{string,1,"a#bc"}],      reia:parse("'a#bc'")),
    % quote string - simple
    ?_assertEqual([{string,1,""}],          reia:parse("\"\" ")),
    ?_assertEqual([{string,1,"abc"}],       reia:parse("\"abc\"")),
    ?_assertEqual([{string,1,"a#bc"}],      reia:parse("\"a#bc\"")),
    ...
    ?_assertEqual([{dstring,1,
                    [ {string,1,"This is an "},
                      ...
                      {string,1," string"}]
                                            ...
                                            reia:parse("'This is an #{\"embedded #{1+1}-level expression\"} string' \n")),
    % tuple
    ?_assertEqual([{tuple,1,[]}],           reia:parse(" ( ) ")),
    ?_assertEqual([{tuple,1,[{integer,1,1}]}],
                                            reia:parse(" ( 1 , ) ")),
    ?_assertEqual([{tuple,1,[{integer,1,1},{integer,1,2},{integer,1,3}]}],
                                            reia:parse(" ( 1 , 2 , 3 ) ")),
    % dictionary
    ?_assertEqual([{dict,1,[]}],           reia:parse(" { } ")),
...
class_and_module_decl_test_() ->
  [ % Class Declarations
    ...
    % Module Declarations
    ?_assertEqual([{module,1,'Mod',[]}],    reia:parse(" module Mod \n end ")),
    ?_assertEqual([{module,1,'NewMod',[{function,2,method,[],{var,1,'_'},[{nil,2}]}]}],
                                            reia:parse(" module NewMod
                                                             def method ()
                                                             end
                                                         end ")),
    ?_assert(true)
